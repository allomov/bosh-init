...
func (m *manager) Upload(extractedStemcell ExtractedStemcell, uploadStage bmui.Stage) (cloudStemcell CloudStemcell, err error) {
	err = uploadStage.Perform("Uploading", func() error {
		...
		if found {
			...
			return bmui.NewSkipStageError(bosherr.Errorf("Stemcell already uploaded: %#v", foundStemcellRecord), "Stemcell already uploaded")
		}
